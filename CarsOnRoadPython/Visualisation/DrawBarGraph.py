...
def draw_bar_graph(history: list[dict]) -> None:
    """
    Draws a coloured bar chart of active EVs, one bar per 30-min tick,
    coloured by day of week.
    """
    ticks       = [entry["tick"]        for entry in history]
    ...
    ax.bar(ticks, active_cars, color=colors, width=0.8, edgecolor="none")
    ...
    ax.set_title("Active EVs on Road — One Week Simulation, Bar Chart (30-min ticks)")
    ax.grid(axis="y", linewidth=0.3, linestyle="--", alpha=0.5)
    plt.tight_layout()
    plt.savefig("ev_graph_bar.png", dpi=150, bbox_inches="tight")
    print("Saved → ev_graph_bar.png")
    plt.show()